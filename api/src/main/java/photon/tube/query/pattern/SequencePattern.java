...
public class SequencePattern<T> {
    ...
    T getUnit(SequencePatternElement element) {
    ...
    public void next(SequencePatternElement<T> element, Consumer<SequencePatternElement<T>> func) {
        ...
        if (element.times >= 2) {
            func.accept(new SequencePatternElement<>(index, getUnit(element), element.times - 1, element.isIndefinite));
        } else {
            index++;
            if (index < length) {
                timesOptions = timesOptionsList.get(index);
                T unit = getUnit(index);
